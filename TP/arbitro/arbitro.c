...
// Defini��o manual de _countof se n�o estiver dispon�vel
#ifndef _countof
#define _countof(array) (sizeof(array) / sizeof(array[0]))
#endif

// Defini��es de Timeout
...
// Estruturas internas do �rbitro (mantidas, pois s�o usadas dentro do SERVER_CONTEXT)
typedef struct {
    TCHAR** palavras;
    DWORD totalPalavras;
...
// ESTRUTURA PRINCIPAL PARA O ESTADO DO SERVIDOR (N�O MAIS GLOBAL)
typedef struct {
    // Gest�o de Jogadores
    ...
    HANDLE hMapFileShm; // Necess�rio para cleanup
    ...
    // Controlo de Logs
    CRITICAL_SECTION csLog; // Esta ser� usada pelas fun��es de Log
} SERVER_CONTEXT;
...
typedef struct {
    ...
    HANDLE hPipeCliente;         // Espec�fico para ThreadClienteConectado, INVALID_HANDLE_VALUE para outras
} THREAD_ARGS;

// ==========================================================================================
// PROT�TIPOS DE FUN��ES INTERNAS (agora, Log* tamb�m recebem csLog)
// ==========================================================================================
...
int _tmain(int argc, TCHAR* argv[]) {
    ...
    SERVER_CONTEXT serverCtx; // Inst�ncia local do contexto do servidor
    ...
    Log(&serverCtx.csLog, _T("[ARBITRO] Iniciando �rbitro..."));
    ...
    THREAD_ARGS argsServico; // Argumentos para threads de servi�o (GestorLetras, Admin)
    ...
    Log(&serverCtx.csLog, _T("[ARBITRO] Servidor pronto. Aguardando conex�es de jogadores em %s"), PIPE_NAME);
    ...
    while (serverCtx.servidorEmExecucao) {
        ...
        if (hPipe == INVALID_HANDLE_VALUE) {
            if (serverCtx.servidorEmExecucao) {
                LogError(&serverCtx.csLog, _T("[ARBITRO] Falha ao criar Named Pipe (inst�ncia): %lu"), GetLastError());
                Sleep(1000);
            }
            continue;
        }
        ...
        if (!fConnected && GetLastError() == ERROR_IO_PENDING) {
            ...
            if (dwWaitResult == WAIT_OBJECT_0) {
                ...
                if (!fConnected) LogError(&serverCtx.csLog, _T("[ARBITRO] GetOverlappedResult falhou ap�s evento: %lu"), GetLastError());
            }
                ...
                LogError(&serverCtx.csLog, _T("[ARBITRO] Erro %lu ao aguardar conex�o no pipe %p."), GetLastError(), hPipe);
                fConnected = FALSE;
            }
        }
        ...
        if (fConnected && serverCtx.servidorEmExecucao) {
            ...
            if (serverCtx.totalJogadoresAtivos < MAX_JOGADORES) {
                ...
                if (argsCliente == NULL) {
                    LogError(&serverCtx.csLog, _T("[ARBITRO] Falha ao alocar mem�ria para THREAD_ARGS."));
                    DisconnectNamedPipe(hPipe);
                    CloseHandle(hPipe);
                }
                else {
                    argsCliente->serverCtx = &serverCtx; // Passa o contexto do servidor
                    argsCliente->hPipeCliente = hPipe;   // Passa o pipe espec�fico do cliente
                ...
                StringCchCopy(msgCheio.data, _countof(msgCheio.data), _T("O jogo est� cheio."));
                DWORD bytesEscritos;
    ...
    Log(&serverCtx.csLog, _T("[ARBITRO] Loop principal de aceita��o de conex�es terminado."));
    if (hThreads[0] != NULL) { WaitForSingleObject(hThreads[0], INFINITE); CloseHandle(hThreads[0]); }
...
// ==========================================================================================
// INICIALIZA��O E ENCERRAMENTO DO SERVIDOR
// ==========================================================================================
BOOL InicializarServidor(SERVER_CONTEXT* ctx) {
    srand((unsigned)time(NULL));

    // csLog j� deve estar inicializada em _tmain ANTES desta fun��o ser chamada.
    // ConfigurarValoresRegistry e CarregarDicionarioServidor usar�o Log, por isso csLog deve estar pronta.

    ConfigurarValoresRegistry(ctx);
    Log(&ctx->csLog, _T("[INIT] Configura��es: MAXLETRAS=%d, RITMO=%ds"), ctx->maxLetrasConfig, ctx->ritmoConfigSegundos);

    if (!CarregarDicionarioServidor(ctx, _T("dicionario.txt"))) {
        LogError(&ctx->csLog, _T("[INIT] Falha ao carregar dicion�rio."));
        return FALSE;
    }

    if (!InicializarMemoriaPartilhadaArbitro(ctx, ctx->maxLetrasConfig)) {
        LogError(&ctx->csLog, _T("[INIT] Falha ao inicializar mem�ria partilhada."));
...
void EncerrarServidor(SERVER_CONTEXT* ctx) {
    ...
    if (ctx->servidorEmExecucao) {
        ...
        StringCchCopy(msgShutdown.data, _countof(msgShutdown.data), _T("O servidor est� a encerrar."));
    ...
    if (ctx->csListaJogadores.DebugInfo != NULL) {
        ...
        LogWarning(&ctx->csLog, _T("[ENCERRAR] Critical section da lista de jogadores n�o inicializada ou j� deletada."));
    }
    ...
    // csLog � deletada em _tmain
}

// ==========================================================================================
// Fun��es de Log (agora recebem csLog explicitamente)
// ==========================================================================================
void Log(CRITICAL_SECTION* csLogParam, const TCHAR* format, ...) {
    // csLogParam DEVE ser inicializado pelo chamador antes da primeira chamada.
    if (csLogParam == NULL || csLogParam->DebugInfo == NULL) { // Fallback se csLogParam n�o estiver pronto/v�lido
...
// ==========================================================================================
// Implementa��es das Fun��es Auxiliares e Threads
// (Todas as fun��es que precisam de log agora usam ctx->csLog)
// ==========================================================================================
    ...
    if (lResult == ERROR_SUCCESS) {
        if (RegQueryValueEx(hKey, REG_MAXLETRAS_NOME, NULL, NULL, (LPBYTE)&dwValor, &dwSize) == ERROR_SUCCESS) {
            if (dwValor > 0 && dwValor <= MAX_LETRAS_TABULEIRO) {
                ...
                LogWarning(&ctx->csLog, _T("[REG] MAXLETRAS (%lu) inv�lido. Usando %d e atualizando registry."), dwValor, MAX_LETRAS_TABULEIRO);
            ...
            LogWarning(&ctx->csLog, _T("[REG] N�o leu MAXLETRAS. Usando padr�o %d e criando/atualizando."), ctx->maxLetrasConfig);
            RegSetValueEx(hKey, REG_MAXLETRAS_NOME, 0, REG_DWORD, (const BYTE*)&ctx->maxLetrasConfig, sizeof(DWORD));
        }
        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, REG_RITMO_NOME, NULL, NULL, (LPBYTE)&dwValor, &dwSize) == ERROR_SUCCESS) {
            if (dwValor > 0 && dwValor < 300) {
                ...
                LogWarning(&ctx->csLog, _T("[REG] RITMO (%lu) inv�lido. Usando padr�o %d e atualizando registry."), dwValor, DEFAULT_RITMO_SEGUNDOS);
                RegSetValueEx(hKey, REG_RITMO_NOME, 0, REG_DWORD, (const BYTE*)&ctx->ritmoConfigSegundos, sizeof(DWORD));
            }
        }
        else {
            LogWarning(&ctx->csLog, _T("[REG] N�o leu RITMO. Usando padr�o %d e criando/atualizando."), ctx->ritmoConfigSegundos);
            RegSetValueEx(hKey, REG_RITMO_NOME, 0, REG_DWORD, (const BYTE*)&ctx->ritmoConfigSegundos, sizeof(DWORD));
        }
        RegCloseKey(hKey);
    }
    else {
        Log(&ctx->csLog, _T("[REG] Chave '%s' n�o encontrada/acess�vel. Criando com valores padr�o."), REGISTRY_PATH_TP);
        if (RegCreateKeyEx(HKEY_CURRENT_USER, REGISTRY_PATH_TP, 0, NULL, REG_OPTION_NON_VOLATILE,
            KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
            RegSetValueEx(hKey, REG_MAXLETRAS_NOME, 0, REG_DWORD, (const BYTE*)&ctx->maxLetrasConfig, sizeof(DWORD));
            RegSetValueEx(hKey, REG_RITMO_NOME, 0, REG_DWORD, (const BYTE*)&ctx->ritmoConfigSegundos, sizeof(DWORD));
            RegCloseKey(hKey);
            Log(&ctx->csLog, _T("[REG] Chave e valores padr�o criados."));
        }
        else {
...
BOOL CarregarDicionarioServidor(SERVER_CONTEXT* ctx, const TCHAR* nomeArquivo) {
    ...
    if (_tfopen_s(&arquivo, nomeArquivo, _T("r, ccs=UTF-8")) != 0 || !arquivo) {
        LogError(&ctx->csLog, _T("[DIC] Erro ao abrir ficheiro de dicion�rio '%s'."), nomeArquivo);
    ...
    if (!dict->palavras) {
        fclose(arquivo);
        LogError(&ctx->csLog, _T("[DIC] Falha ao alocar mem�ria inicial para dicion�rio."));
        DeleteCriticalSection(&dict->csDicionario);
        return FALSE;
    }
    ...
    while (_fgetts(linha, MAX_WORD + 1, arquivo)) {
        ...
        if (dict->totalPalavras >= capacidade) {
            ...
            if (!temp) {
                LogError(&ctx->csLog, _T("[DIC] Falha ao realocar mem�ria para dicion�rio."));
        ...
        if (!dict->palavras[dict->totalPalavras]) {
            LogError(&ctx->csLog, _T("[DIC] Falha ao alocar mem�ria para a palavra '%s'."), linha);
            for (DWORD i = 0; i < dict->totalPalavras; i++) free(dict->palavras[i]);
            free(dict->palavras); dict->palavras = NULL;
            fclose(arquivo);
            DeleteCriticalSection(&dict->csDicionario);
    ...
    fclose(arquivo);
    Log(&ctx->csLog, _T("[DIC] Dicion�rio '%s' carregado com %lu palavras."), nomeArquivo, dict->totalPalavras);
    if (dict->totalPalavras == 0) {
        LogWarning(&ctx->csLog, _T("[DIC] Dicion�rio carregado est� vazio!"));
    }
    return TRUE;
}
...
BOOL InicializarMemoriaPartilhadaArbitro(SERVER_CONTEXT* ctx, int maxLetras) {
    if (maxLetras <= 0 || maxLetras > MAX_LETRAS_TABULEIRO) {
        LogError(&ctx->csLog, _T("[SHM] maxLetras inv�lido (%d) para inicializar mem�ria partilhada."), maxLetras);
        return FALSE;
    }
    ...
    Log(&ctx->csLog, _T("[SHM] Mem�ria partilhada '%s' e evento '%s' inicializados."), SHM_NAME, EVENT_SHM_UPDATE);
    return TRUE;
}
...
void LimparMemoriaPartilhadaArbitro(SERVER_CONTEXT* ctx) {
    ...
    ctx->hMutexShm = NULL;
    Log(&ctx->csLog, _T("[SHM] Mem�ria partilhada e objetos de sync limpos."));
}
    ...
    while (ctx->servidorEmExecucao) {
        ...
        if (_tcscmp(comando, _T("listar")) == 0) {
            ...
            if (ctx->ritmoConfigSegundos > 1) {
                ...
                Log(&ctx->csLog, _T("[ADMIN] Ritmo j� est� no m�nimo (1 segundo)."));
            }
            ReleaseMutex(ctx->hMutexShm);
        }
...
DWORD WINAPI ThreadClienteConectado(LPVOID param) {
    ...
    if (ReadFile(hPipe, &msg, sizeof(MESSAGE), &bytesLidos, &olRead) || GetLastError() == ERROR_IO_PENDING) {
        ...
        if (bytesLidos == sizeof(MESSAGE) && _tcscmp(msg.type, _T("JOIN")) == 0) {
            ...
            if (idxExistente != -1) {
                LeaveCriticalSection(&ctx->csListaJogadores);
                LogWarning(&ctx->csLog, _T("[CLIENT_THREAD %p] Username '%s' j� em uso."), hPipe, usernameEsteCliente);
            ...
            LogError(&ctx->csLog, _T("[CLIENT_THREAD %p] Primeira msg n�o foi JOIN ou bytes errados. Desconectando."), hPipe);
            goto cleanup_cliente_thread;
        }
    }
    else {
        LogError(&ctx->csLog, _T("[CLIENT_THREAD %p] Falha ao ler 1� msg: %lu."), hPipe, GetLastError());
        goto cleanup_cliente_thread;
    }
    ...
    while (ctx->servidorEmExecucao && esteClienteAtivo) {
        ...
        if (!sucessoLeitura && dwErrorRead == ERROR_IO_PENDING) {
            ...
            if (!GetOverlappedResult(hPipe, &olRead, &bytesLidos, FALSE)) {
                LogError(&ctx->csLog, _T("[CLIENT_THREAD %p] GOR falhou ap�s ReadFile para '%s': %lu."), hPipe, usernameEsteCliente, GetLastError());
                esteClienteAtivo = FALSE; break;
            }
            sucessoLeitura = TRUE;
        }
        ...
        if (sucessoLeitura && bytesLidos == sizeof(MESSAGE)) {
            ...
            if (meuIndiceNaLista == -1 || meuIndiceNaLista >= MAX_JOGADORES ||
                ...
                LeaveCriticalSection(&ctx->csListaJogadores);
                LogWarning(&ctx->csLog, _T("[CLIENT_THREAD %p] Jogador '%s' tornou-se inv�lido ou pipe desatualizado. Encerrando."), hPipe, usernameEsteCliente);
                esteClienteAtivo = FALSE; break;
            }
...
void RemoverJogador(SERVER_CONTEXT* ctx, const TCHAR* username, BOOL notificarClienteParaSair) {
    ...
    for (DWORD i = 0; i < MAX_JOGADORES; ++i) {
        if (ctx->listaJogadores[i].ativo && _tcscmp(ctx->listaJogadores[i].username, username) == 0) {
            Log(&ctx->csLog, _T("[JOGADOR] '%s' (idx %lu) encontrado para remo��o."), username, i);
            ctx->listaJogadores[i].ativo = FALSE;
    ...
    if (idxRemovido != -1) {
        if (notificarClienteParaSair && hPipeDoRemovido != INVALID_HANDLE_VALUE) {
            ...
            StringCchCopy(msgKick.data, _countof(msgKick.data), _T("Voc� foi desconectado/exclu�do."));
            DWORD bw;
        ...
        LogWarning(&ctx->csLog, _T("[JOGADOR] '%s' n�o encontrado para remo��o ou j� inativo."), username);
    }
}

int EncontrarJogador(SERVER_CONTEXT* ctx, const TCHAR* username) {
    // Assume que ctx->csListaJogadores J� EST� DETIDO pelo chamador (como indicado no prot�tipo original)
    for (DWORD i = 0; i < MAX_JOGADORES; ++i) {
        if (ctx->listaJogadores[i].ativo && _tcscmp(ctx->listaJogadores[i].username, username) == 0) {
            return (int)i;
        }
...
BOOL ValidarPalavraJogo(THREAD_ARGS* argsClienteThread, const TCHAR* palavraSubmetida, const TCHAR* usernameJogador) {
    ...
    if (!ctx->jogoRealmenteAtivo) {
        ...
        StringCchCopy(resposta.data, _countof(resposta.data), _T("O jogo n�o est� ativo."));
    }
    else {
        ...
        if (!existeNoDicionario) {
            StringCchCopy(resposta.type, _countof(resposta.type), _T("WORD_INVALID"));
            StringCchPrintf(resposta.data, _countof(resposta.data), _T("'%s' n�o existe no dicion�rio."), palavraSubmetida);
            ...
            if (podeFormar) {
                ...
                StringCchPrintf(resposta.data, _countof(resposta.data), _T("'%s' v�lida! +%.1f pts."), palavraSubmetida, pontosAlterados);
                ...
                StringCchCopy(resposta.type, _countof(resposta.type), _T("WORD_INVALID"));
                StringCchPrintf(resposta.data, _countof(resposta.data), _T("'%s' n�o pode ser formada com as letras atuais."), palavraSubmetida);
    ...
    if (pontosAlterados != 0.0f) {
        EnterCriticalSection(&ctx->csListaJogadores);
        int idxJogador = EncontrarJogador(ctx, usernameJogador); // Assume csListaJogadores j� detido
        if (idxJogador != -1) {
            ctx->listaJogadores[idxJogador].pontos += pontosAlterados;
            Log(&ctx->csLog, _T("[JOGO] %s (%s): %+.1f pts. Total: %.1f"), usernameJogador,
                (_tcscmp(resposta.type, _T("WORD_VALID")) == 0 ? _T("acertou") : _T("errou/inv�lida")),
...
void VerificarEstadoJogo(SERVER_CONTEXT* ctx) {
    ...
    if (deveEstarAtivo && !estavaAtivo) {
        ...
        Log(&ctx->csLog, _T("[JOGO] O jogo come�ou/recome�ou! (%lu jogadores)"), totalJogadoresAtivosSnapshot);
        ...
        StringCchCopy(msgJogoIniciou.data, _countof(msgJogoIniciou.data), _T("O jogo come�ou! Boa sorte!"));
        ...
        if (ctx->listaJogadores[i].ativo && ctx->listaJogadores[i].hPipe != INVALID_HANDLE_VALUE) {
            if (skipUsername == NULL || _tcscmp(ctx->listaJogadores[i].username, skipUsername) != 0) {
                ...
                if (olNotify.hEvent == NULL) {
                    LogError(&ctx->csLog, _T("[NOTIFICAR] Falha ao criar evento para notifica��o para %s."), ctx->listaJogadores[i].username);
                    continue;
                }
                ...
                if (!WriteFile(ctx->listaJogadores[i].hPipe, msgAEnviar, sizeof(MESSAGE), &bytesEscritos, &olNotify)) {
                    if (GetLastError() == ERROR_IO_PENDING) {
                        if (WaitForSingleObject(olNotify.hEvent, 1000) == WAIT_OBJECT_0) {
                            ...
                            LogWarning(&ctx->csLog, _T("[NOTIFICAR] Timeout/Erro ao enviar notifica��o para %s. Cancelando IO."), ctx->listaJogadores[i].username);
                            CancelIoEx(ctx->listaJogadores[i].hPipe, &olNotify);
                        }
                    }
                    else {
                        LogWarning(&ctx->csLog, _T("[NOTIFICAR] Falha ao enviar notifica��o para %s (pipe %p): %lu"), ctx->listaJogadores[i].username, ctx->listaJogadores[i].hPipe, GetLastError());
                    }
                }
                else {
                    GetOverlappedResult(ctx->listaJogadores[i].hPipe, &olNotify, &bytesEscritos, FALSE);
                    if (bytesEscritos != sizeof(MESSAGE)) {
                        // LogWarning(&ctx->csLog, _T("[NOTIFICAR] WriteFile S�ncrono (com overlapped) enviou %lu bytes em vez de %zu para %s."), bytesEscritos, sizeof(MESSAGE), ctx->listaJogadores[i].username);
                    }
                }
